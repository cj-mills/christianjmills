...
The `PreprocessResNet` method returns a Texture2D with an HDR texture format. The switch to HDR texture formats means we need to destroy the `tempTex` variable as it's not compatible. Fortunately, we can reuse the `imageTexture` variable that we emptied.
